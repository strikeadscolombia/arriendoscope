  ...
  async scrape(cities) {
    ...
    for (const city of cities) {
      for (let page = 0; page < this.maxPages; page++) {
        try {
          const rawItems = await this.fetchPage(city, page);
          if (!rawItems || rawItems.length === 0) break;

          for (const item of rawItems) {
            try {
              const parsed = this.parseListing(item, city);
          ...
          logger.info(`[${this.name}] ${city} page ${page}: ${rawItems.length} items`);
          await this.delay(2000 + Math.random() * 3000);
        } catch (err) {
          logger.error(`[${this.name}] ${city} page ${page}: ${err.message}`);
          break;
        }
      }
    }
